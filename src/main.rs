...
fn lock_picking_challenge(secret_combination: &str) -> bool{
    use std::io;
...
fn main() {
    ...
    let challenges = [
        ...
        Challenge { //0
            ...
            paths: vec![
                Path{target:ChallengeID(1), pre_message: Some("Door".into()), post_message:Some("You slip through the door and find yourself in a dark basement.".into()), required_resource: None}, // No required resource for this path
                Path{target:ChallengeID(1), pre_message: Some("Vent".into()), post_message:Some("You shimmy through an exterior vent and crawl through the ducts.".into()), required_resource: None},
                Path{target:ChallengeID(1), pre_message: Some("Window".into()), post_message:Some("You climb through the window and find yourself in a dark storage room.".into()), required_resource: None},
            ],
            password: None,
            
        },
        Challenge { // 1
            desc: "Oh no, you are locked in... What do you do?".into(),
            // resource checking
            paths:vec![
                Path{target:ChallengeID(2), pre_message: Some("Eat a snack".into()), post_message:Some(format!("{} \nThe pungent smell of banana fills the room, so much so that you can almost see it drifting out under the door.", ascii_art::BANANA).into()), required_resource: Some(available_resources[6].clone())},
                Path{target:ChallengeID(14), pre_message: Some("Pick the lock to the door".into()), post_message: None, required_resource: Some(available_resources[2].clone())},
                Path{target:ChallengeID(2), pre_message: Some("Shine your flashlight".into()), post_message:Some("You turn on the flashlight and discover a secret door".into()), required_resource: Some(available_resources[0].clone())},
            ],
            password: None,
        },
        Challenge { // 2 Footsteps
            desc: "You exit into a hallway and hear footsteps coming your way.".into(),
            paths:vec![
                Path{target:ChallengeID(9), pre_message: Some("Run blindly away".into()), post_message:Some("The guard's partner has caught you.".into()), required_resource: None},
                Path{target:ChallengeID(10), pre_message: Some("Hide behind a nearby priceless statue".into()), post_message:Some("The security guard stops next to you...you need a distraction.".into()), required_resource: None},
                Path{target:ChallengeID(4), pre_message: Some("Run towards the sound".into()), post_message:Some("You approach the security guard and...".into()), required_resource: None},
            ],
            password: None,
        },
        Challenge { // 3 Evaded guard
            desc: "You've successfully evaded the guard! Where do you go next?".into(),
            paths:vec![
                Path{target:ChallengeID(5), pre_message: Some("Tourist shop".into()), post_message:Some("You enter the gift shop...".into()), required_resource: None},
                Path{target:ChallengeID(6), pre_message: Some("Security office".into()), post_message:Some("You enter the security office...".into()), required_resource: None},
                Path{target:ChallengeID(9), pre_message: Some("Cafe".into()), post_message:Some("You enter the cafe and find a group of guards sitting at one of the tables eating doughnuts. You're toast.".into()), required_resource: None},
            ],
            password: None, 
        },
        Challenge { // 4 talk to guard
            desc: "You run smack into the guard. Do something quickly!".into(),
            paths:vec![
                Path{target:ChallengeID(7), pre_message: Some("Pretend to be a ghost".into()), post_message:Some(format!("{} \nYou quickly slap on some pale foundation and white powder. You're the splitting image of Casper. OOoooOooooOOO. \nThe guard screams with terror, runs away, and leaves you with an open path to the painting.", ascii_art::CASPER).into()), required_resource: Some(available_resources[4].clone())},
                Path{target:ChallengeID(9), pre_message: Some("Pretend to be lost".into()), post_message:Some("The guard didn't believe your act and called the police. You should enroll in an acting class.".into()), required_resource: None},
                Path{target:ChallengeID(7), pre_message: Some("Befriend the guard".into()), post_message:Some("Your charming wit convinced the guard your mission was worth it.".into()), required_resource: None},
            ],
            password: None, 
        },
        Challenge { // 5 tourist shop
            desc: "You are at the tourist shop..what do you do?".into(),
            paths:vec![
                Path{target:ChallengeID(7), pre_message: Some("Try on merch".into()), post_message:Some("The merch looks amazing on you, but the guard catches you while you are stashing your bag. You didn't have room to carry the painting anyways :(".into()), required_resource: None},
                Path{target:ChallengeID(7), pre_message: Some("Look at the tourist maps".into()), post_message:Some("You look at the maps and find directions to the painting".into()), required_resource: None},
                Path{target:ChallengeID(9), pre_message: Some("Raid the cash register".into()), post_message:Some("You found money but not the painting, you did not complete the mission".into()), required_resource: None},
            ],
            password: None, 
        },
        Challenge { // 6 security office
            desc: "You sneak into the security office. There are lots of things lying around and cameras on the wall. What do you do?".into(),
            paths:vec![
                Path{target:ChallengeID(13), pre_message: Some("Inspect the camera system".into()), post_message:None, required_resource: None},
                Path{target:ChallengeID(7), pre_message: Some("Look at the posters on the wall".into()), post_message:Some("Congrats you found blueprints that show you where the painting is. You sneakily walk through the hallways avoiding any remaining guards until you arrive at the world famous piece of art.".into()), required_resource: None},
                Path{target:ChallengeID(9), pre_message: Some("Open the fridge".into()), post_message:Some("Food? At a time like this?".into()), required_resource: None},
            ],
            password: None, 
        },
        Challenge { // 7 painting
            desc: format!("{}\n You finally arrive at the painting, how do you get it off the wall?", ascii_art::CECIL).into(),
            paths:vec![
                Path{target:ChallengeID(8), pre_message: Some("Cut it out".into()), post_message:Some("While doing irreparable damage to the painting, you manage to get it out of the frame in the fastest time.".into()), required_resource: Some(available_resources[3].clone())},
                Path{target:ChallengeID(12), pre_message: Some("Pick the lock".into()), post_message:None, required_resource: Some(available_resources[2].clone())},
                Path{target:ChallengeID(8), pre_message: Some("Dissolve the glue with acetone".into()), post_message:Some("You paint the glue on the back of the frame. The painting remains unharmed and you can sell it for maximum profit!".into()), required_resource: Some(available_resources[4].clone())},
            ],
            password: None, 
        },
        ...
        Challenge { // 10 distraction
            ...
            paths:vec![
                Path{target:ChallengeID(3), pre_message: Some("Throw a marble in another direction to get them off your tail.".into()), post_message:Some("The marble bounces off into another room and the guard runs after the noise.".into()), required_resource: Some(available_resources[5].clone())},
                Path{target:ChallengeID(3), pre_message: Some("Strategically place your banana peel so that they slip.".into()), post_message:Some("The guard slips on the banana peel and you manage to run off while they're down.".into()), required_resource: Some(available_resources[6].clone())},
                Path{target:ChallengeID(9), pre_message: Some("Attack them with your knife".into()), post_message:Some("The guard knows martial arts. They pin you to the ground easily and call the police.".into()), required_resource: Some(available_resources[3].clone())},
            ],
            password: None, 
        },
        Challenge { // 11 Run towards guard
            ...
            paths:vec![
                Path{target:ChallengeID(9), pre_message: Some("Attack them your knife".into()), post_message:Some("Really? You're a CS major, stop kidding yourself.".into()), required_resource: Some(available_resources[3].clone())},
                Path{target:ChallengeID(3), pre_message: Some("Blind them with your sand".into()), post_message:Some("The guard screams and claws at their eyes and you manage to escape during their suffering.".into()), required_resource: Some(available_resources[7].clone())},
                Path{target:ChallengeID(4), pre_message: Some("Cautiously talk to them".into()), post_message:Some("".into()), required_resource: Some(available_resources[0].clone())},
            ],
            password: None, 
        },
        Challenge { // 12 lock picking
            desc: "You are picking the lock.".into(),
            paths:vec![Path{target:ChallengeID(8), pre_message: None, post_message: None, required_resource: None},],
            password: None, 
        },
        ...
        Challenge { //14 initial door lock
            desc: "The system is password protect but theres a hint! I have cities, but no houses. I have mountains, but no trees. I have water, but no fish. What am I? (Format like 'A ___'".into(),
            paths:vec![],
            password: None, 
        },
    ...
    loop {
        ...
        if !options_printed{
            println!("{} \nYou had insufficient resources to continue moving forward in the heist. Please come better prepared next time.", ascii_art::JAIL);
            println!("You lose.");
            break;
        }
            ...
            Ok(index) => {
                if index >= 1 && index <= current_challenge.paths.len() {
                    ...
                    if selected_path.target.0 == 12 {
                        ...
                        let lock_picked = lock_picking_challenge("1887");
                        if lock_picked {
                            println!("-----------------------------------------------------------------------------------------");
                    ...
                    if selected_path.target.0 == 14 {
                        // check if the  current is the lock-picking challenge - 13
                        println!("It looks like the lock requires both a key and a password. Good thing you have your lock picking set! How are we going to figure out the password...? \n \n{} \nSomething has been faintly scratched into the wall next to the lock. Maybe it's a clue?", ascii_art::NUMBER);
                        let lock_picked = lock_picking_challenge("181G");
                        if lock_picked {
                            println!("The lock pops open with a satisying click");
                            println!("You pick the lock on the door and it swings open too fast for you to catch it before it slams with a CRASH into the wall.");
                            // Move to the next challenge 
                            current_challenge_id = ChallengeID(2);
                        } else {
                            //  return to original challenge
                            current_challenge_id = ChallengeID(1);
                        }
                    }
